...
from ariadne import graphql_sync, make_executable_schema, load_schema_from_path, snake_case_fallback_resolvers, \
    format_error, ScalarType, EnumType, UnionType, InterfaceType, SchemaDirectiveVisitor
from ariadne.constants import PLAYGROUND_HTML
from flask import Flask, request, jsonify
from graphql import GraphQLError, default_field_resolver, GraphQLSchema

from graph.mutation import mutation, ApiException
from graph.query import query, user
...
datetime_scalar = ScalarType("Datetime", serializer=serialize_datetime, value_parser=parse_datetime_value)
...
schema = make_executable_schema(type_defs,
                                [query, user, bindable, error, search_result, user_status, datetime_scalar, mutation,
                                 snake_case_fallback_resolvers],
                                directives={
                                    'needsPermission': PermissionDirective,
                                })
app = Flask(__name__)
...
def graphql_server():
    ...
    def my_format_error(error: GraphQLError, debug: bool = False) -> dict:
        if not debug:
            if isinstance(error.original_error, GraphQLError) and isinstance(error.original_error.original_error,
                                                                             ApiException):
